...
public class Duplicator implements PlugIn, TextListener {
	...
	public void run(String arg) {
		...
		String newTitle = WindowManager.getUniqueName("DUP_"+title);
	...
	public ImagePlus run(ImagePlus imp) {
   		if (Recorder.record) Recorder.recordCall("impD = new Duplicator().run(imp);");
		...
		ImagePlus imp2 = new ImagePlus("DUP_"+imp.getTitle(), stack2);
	...
	ImagePlus duplicateImage(ImagePlus imp) {
		...
		ImagePlus imp2 = new ImagePlus("DUP_"+imp.getTitle(), ip2);
		String info = (String)imp.getProperty("Info");
		if (info!=null)
			imp2.setProperty("Info", info);
		if (imp.getStackSize()>1) {
	...
	public ImagePlus run(ImagePlus imp, int firstSlice, int lastSlice) {
		...
		if (tseries)
   		...
   		if (Recorder.record) Recorder.recordCall("impD = new Duplicator().run(imp, "+firstSlice+", "+lastSlice+");");
		return imp2;
	}
	...
	public ImagePlus run(ImagePlus imp, int firstC, int lastC, int firstZ, int lastZ, int firstT, int lastT) {
		...
		imp2.setOpenAsHyperStack(true);
   		if (Recorder.record)
   			Recorder.recordCall("impD = new Duplicator().run(imp, "+firstC+", "+lastC+", "+firstZ+", "+lastZ+", "+firstT+", "+lastT+");");
		return imp2;
	}
