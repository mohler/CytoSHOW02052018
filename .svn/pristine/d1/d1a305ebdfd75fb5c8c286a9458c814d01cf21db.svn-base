...
//        	size = (int)(3/mag);
	...
	public void addChar(char c) {
		if (imp==null || IJ.getToolName() != "text") return;
		if (!(c>=' ' || c=='\b' || c=='\n')) return;
		if (firstChar) {
			cline = 0;
			...
			for (int l=0; l<theText.length;l++)
			if (theText[l]!=null)
				cline = l;
		}
		if ((int)c=='\b') {
	...
	void drawText(Graphics g) {
		...
		while (i<MAX_LINES && theText[i]!=null) {
			switch (justification) {
				case LEFT:
					g.drawString(theText[i].replaceAll("( zL| tL)", ""), sx, sy+fontHeight-descent);
					break;
				case CENTER:
					int tw = metrics.stringWidth(theText[i]);
					g.drawString(theText[i].replaceAll("( zL| tL)", ""), sx+(sw-tw)/2, sy+fontHeight-descent);
					break;
				case RIGHT:
					tw = metrics.stringWidth(theText[i]);
					g.drawString(theText[i].replaceAll("( zL| tL)", ""), sx+sw-tw, sy+fontHeight-descent);
					break;
			}
			i++;
			sy += fontHeight;
	...
	*/
	...
	protected void searchWormbase(){
		String[] searchTextChunks = theText[0].replace("\'", "\\\'").split(" ");
		String searchText = "";
		...
		if ((IJ.controlKeyDown() || IJ.altKeyDown()) && IJ.shiftKeyDown()) {
			...
			IJ.runPlugIn("ij.plugin.BrowserLauncher",
					"http://wormatlas.org/search_results.html?cx=016220512202578422943%3Amikvfhp2nri&cof=FORID%3A10&ie=UTF-8&q="
							+ searchText + "&siteurl=wormatlas.org%252F" );
...
//		heightd = nLines*fontHeight+2;
	...
	public synchronized Object clone() {
		...
		if (ic != null && ic.getGraphics() != null)
			this.updateBounds(ic.getGraphics());
		tr.getBounds().setBounds(this.x, this.y, this.width, this.height);
		if (ic != null && ic.getGraphics() != null)
	...
	public void setText(String newText) {
		Arrays.fill(theText,null);
		String[] editText = newText.split("\\|");
		for (int l=0; l<editText.length; l++) {
			theText[l] = editText[l];
		}
		updateBounds(null);
		updateText();
	}
