...
public class StackLabeler implements ExtendedPlugInFilter, DialogListener {
	...
	public int setup(String arg, ImagePlus imp) {
		...
		this.imp = imp;
		if (imp.getRoi()!=null){
			x= (int)imp.getRoi().getBounds().getMaxX();
			y= (int)imp.getRoi().getBounds().getMaxY();					
		}
		return flags;
	}
    ...
    public int showDialog(ImagePlus imp, String command, PlugInFilterRunner pfr) {
		...
		if (roi.width<ip.getWidth() || roi.height<ip.getHeight()) {
			x= (int)roi.getBounds().getMaxX();
			y= (int)roi.getBounds().getMaxY();		
		...
		gd.setInsets(2, 5, 0);
		gd.addChoice("Dimension:", new String[]{"Channels", "Slices", "Frames"}, "Frames");
	...
	public boolean dialogItemChanged(GenericDialog gd, AWTEvent e) {
		...
		double[] range = getRange(gd, 1, dimension=="Channels"?imp.getNChannels():
											(dimension=="Slices"?imp.getNSlices():
												imp.getNFrames()));
...
//		if (imp.isHyperStack()) n = updateIndex(n);
		int image = imp.getFrame();
		int n = image - 1;
		if (virtualStack) {
			int nFrames = 0;
			if (dimension == "Channels")
				nFrames = imp.getNChannels();
			else if (dimension == "Slices")
				nFrames = imp.getNSlices();
			else if (dimension == "Frames")
				nFrames = imp.getNFrames();

			if (previewing) nFrames = 1;
...
//		}
		if (useOverlay) {
			...
			if (frame>=firstFrame&&frame<=lastFrame) {
				...
				Roi roi = new TextRoi(xloc, yloc, s, font);
