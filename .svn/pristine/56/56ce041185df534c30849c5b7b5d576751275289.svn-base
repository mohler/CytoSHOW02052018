	...
	*/
	...
	public static synchronized void log(String s) {
		...
		if (logPanel==null && ij!=null) {
			TextWindow logWindow = new TextWindow("Too Much Info ;) Window", "", 400, 250);
	...
	static void handleLogCommand(String s) {
		if (s.equals("\\Closed"))
			...
			Frame f = WindowManager.getFrame("Too Much Info ;) Window");
...
/** Clears the "Results" window and sets the column headings to
	...
	/** Displays an "out of memory" message to the "Too Much Info ;) Window" window. */
...
//			IJ.noImage();
	...
	*/
	 public static ImagePlus createImage(String title, int width, int height, int depth, int bitdepth) {
		return NewImage.createImage(title, width, height, depth, bitdepth, NewImage.FILL_BLACK, false);
	 }
	 ...
	 public static ImagePlus createImage(String title, String type, int width, int height, int depth) {
		...
		return NewImage.createImage(title, width, height, depth, bitDepth, options, false);
	}
