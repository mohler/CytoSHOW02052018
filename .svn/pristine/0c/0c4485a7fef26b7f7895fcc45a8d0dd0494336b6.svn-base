...
*/
public class QTMovieOpenerMultiMod implements PlugIn, QDConstants, StdQTConstants, MovieDrawingComplete {
	...
	private boolean viewOverlay = false;
	private boolean sideSideStereo = false, redCyanStereo = false;
 	private boolean horizontal;
	private boolean grid;
	
/*  */	
//constructor for calling by static method:
     QTMovieOpenerMultiMod(String pathlist, String options) {
...
//constructor for calling by static method:
     QTMovieOpenerMultiMod(String options, int one) {
		Macro.setOptions(options);
		this.pathlist = null;
         run("");
     }
...
//constructor for calling by static method:
     public QTMovieOpenerMultiMod(String pathlist) {
    	 if (IJ.debugMode) IJ.log("this.redCyanStereo" + this.redCyanStereo +"this.viewOverlay"+ this.viewOverlay);

         this.pathlist = pathlist; //set instance variable
         run("");
     }

public QTMovieOpenerMultiMod(String pathListStereo, boolean redCyanStereo2, boolean viewOverlay2) {
	if (IJ.debugMode) IJ.log("pathListStereo" + pathListStereo +"this.pathlist"+ this.pathlist);
	if (IJ.debugMode) IJ.log("this.redCyanStereo" + this.redCyanStereo +"this.viewOverlay"+ this.viewOverlay);
    this.pathlist = pathListStereo;
    this.redCyanStereo = redCyanStereo2;
	this.viewOverlay = viewOverlay2;
	run("");
}

//static method
     public static void runQT_Movie_OpenerMultiMod(String pathlist,  String options) {
         new QTMovieOpenerMultiMod(pathlist, options);
		 //return ("Pathlist, Options");
     }

//static method
     public static void runQT_Movie_OpenerMultiMod(String options, int one) {
         new QTMovieOpenerMultiMod(options, 1);
		 //return ("Options only");
     }

//static method
     public static void runQT_Movie_OpenerMultiMod(String pathlist) {
    	 
         new QTMovieOpenerMultiMod(pathlist);
		 //return ("Pathlist only");
         
     }
   //static method called by ij.plugin.DragAndDrop if a URL is intended to give RedCyanStero
     public static void runQT_Movie_OpenerMultiMod(String pathListStereo, boolean redCyanStereo, boolean viewOverlay) {
    	 new QTMovieOpenerMultiMod(pathListStereo, redCyanStereo, viewOverlay);
...
/*  */
	...
	public void run(String arg) {
		...
		try {
			...
			IJ.error("Requires QuickTime for Java, available \n"
		...
		gd.addCheckbox("View in Overlay?", viewOverlay);		
		gd.addCheckbox("Side-by-Side Stereo, for uncrossed eyes?", false);		
		gd.addCheckbox("Red-Cyan Stereo, for funny glasses?", redCyanStereo);				
		gd.addCheckbox("Horizontal Montage?", true);
		gd.addCheckbox("Grid Layout??", false);		
				
		if (pathlist.contains("|") ) {  /************ CASE IF MORE THAN ONE MOVIE IS SELECTED ***********/
			gd.showDialog();
			if (gd.wasCanceled())
				return;
		...
		} else if (pathlist != null) {	/******** CASE IF ONLY ONE MOVIE IS SELECTED ***********/
			grayscale = true;
			virtualStack = true;
			multiVStack = true;
	...
	static String[] ParsePathsFromInputString(String pathlist) {
		
		if (pathlist==null) return null;								// True if no Movie files opened.
		//if (IJ.debugMode) IJ.log(pathlist);
/**/			
		String[] path = pathlist.split("\\|");				// Makes String[] array of all of the file paths to movies using | as the separator to split with later.
		//if (IJ.debugMode) IJ.log(path[0]);
...
/*************BUILD THE imp ABOVE WITHIN THE CONSTRUCTOR OF MultiQTVirtualStack, RATHER THAN IN THIS LINE OF THIS CODE??	*/			
			...
			if ( imp != null ) {
				if (virtualHyperStack) {
					convertStackToHS(imp, vstack, eightBit);
				}

				if (!virtualHyperStack) imp.show();
				if (imp.getNChannels() == 1 && eightBit)
					IJ.run("Grays");
...
*/
    ...
    void convertStackToHS(ImagePlus imp, VirtualStack vstack, boolean eightBit) {
		...
		if (pathlist.contains("|") ) {/********** CASE OF MORE THAN ONE MOVIE  ************/
...
*/
			...
			if (nChannels>0 && imp.getBitDepth()!=24) {
            	if (nChannels <1) {
            		imp2 = new CompositeImage(imp, CompositeImage.GRAYSCALE); //Case of a single channel movie.  Must be in Gray mode to accept LUTs.
            	}else{
			...
			Frame displayAdjuster = WindowManager.getFrame("Display");
			if (displayAdjuster == null)
				displayAdjuster = WindowManager.getFrame("Display Channels");
			if (displayAdjuster != null)
				displayAdjuster.setLocation(screenDimension.width-(165),100);
        ...
        if (imp.getNChannels() <= 1) {
        	((MultiQTVirtualStack) vstack).setChannelLUTIndex(0, 3) ;
//			IJ.run("Multi-Channel Controller...");
			...
			if (mcf == null) {
				...
				mcf = imp.getMultiChannelController();
			}
			if (mcf != null) {
				...
				if (redCyanStereo) {
					for (int c=0; c<((MultiQTVirtualStack) vstack).imp.getNChannels(); c = c+2) {
						...
						((MultiQTVirtualStack) vstack).setShiftSingleMovieZPosition(c, 1);
					}
				}
				if (sideSideStereo)  {
					for (int c=0; c<((MultiQTVirtualStack) vstack).imp.getNChannels(); c = c+2) {
