...
public class NewImage {
	...
	static boolean createStack(ImagePlus imp, ImageProcessor ip, int nSlices, int type, int options, boolean vStack) {
		int fill = getFill(options);
		Color virtualFillColor = fill == FILL_WHITE?Color.white:Color.black;
		...
		if (!vStack && (options&CHECK_AVAILABLE_MEMORY)!=0) {
		...
		if (!vStack) 
			...
			stack = new VirtualStack(width,  height,  null,  null,  vStack, virtualFillColor); 
		int inc = nSlices/40;
		if (inc<1) inc = 1;
		if (!vStack)
			IJ.showStatus("Allocating "+size2+". Press 'Esc' to abort.");
		else
			IJ.showStatus("Virtualizing "+size2+". Press 'Esc' to abort.");
...
//		new StackWindow(imp);
	...
	public static ImagePlus createByteImage(String title, int width, int height, int slices, int options, boolean vStack) {
		int fill = getFill(options);
		...
		if (slices>1) {
			boolean ok = createStack(imp, ip, slices, GRAY8, options, vStack);
			if (!ok) imp = null;
		}
		return imp;
	}

	public static ImagePlus createRGBImage(String title, int width, int height, int slices, int options, boolean vStack) {
		int fill = getFill(options);
		int size = getSize(width, height);
		if (size<0) return null;
		...
		imp.setProcessor(title, ip);
		if (slices>1) {
			boolean ok = createStack(imp, ip, slices, RGB, options, vStack);
			if (!ok) imp = null;
		}
		return imp;
	}
	 ...
	 * @param stack */
	public static ImagePlus createShortImage(String title, int width, int height, int slices, int options, boolean vStack) {
		int fill = getFill(options);
		int size = getSize(width, height);
		if (size<0) return null;
		...
		imp.setProcessor(title, ip);
		if (slices>1) {
			boolean ok = createStack(imp, ip, slices, GRAY16, options, vStack);
			if (!ok) imp = null;
		}
	...
	*/
	public static ImagePlus createUnsignedShortImage(String title, int width, int height, int slices, int options) {
		return createShortImage(title, width, height, slices, options, vStack);
	}

	public static ImagePlus createFloatImage(String title, int width, int height, int slices, int options, boolean vStack) {
		int fill = getFill(options);
		int size = getSize(width, height);
		if (size<0) return null;
		...
		imp.setProcessor(title, ip);
		if (slices>1) {
			boolean ok = createStack(imp, ip, slices, GRAY32, options, vStack);
			if (!ok) imp = null;
		}
	...
	public static void open(String title, int width, int height, int nSlices, int type, int options, boolean vStack) {
		int bitDepth = 8;
		if (type==GRAY16) bitDepth = 16;
		else if (type==GRAY32) bitDepth = 32;
		else if (type==RGB) bitDepth = 24;
		long startTime = System.currentTimeMillis();
		ImagePlus imp = createImage(title, width, height, nSlices, bitDepth, options, vStack);
		if (imp!=null) {
	...
	public static ImagePlus createImage(String title, int width, int height, int nSlices, int bitDepth, int options, boolean vStack) {
		ImagePlus imp = null;
		switch (bitDepth) {
			case 8: imp = createByteImage(title, width, height, nSlices, options, vStack); break;
			case 16: imp = createShortImage(title, width, height, nSlices, options, vStack); break;
			case 32: imp = createFloatImage(title, width, height, nSlices, options, vStack); break;
			case 24: imp = createRGBImage(title, width, height, nSlices, options, vStack); break;
	...
	boolean showDialog() {
		...
		gd.addCheckbox("Virtual Stack", vStack);
		gd.showDialog();
		if (gd.wasCanceled())
			return false;
	...
	void openImage() {
		if (!showDialog())
			return;
		try {open(name, width, height, channels*slices*frames, type, fillWith, vStack);}
	...
	public static void savePreferences(Properties prefs) {
		...
		prefs.put(CHANNELS, Integer.toString(channels));
		prefs.put(SLICES, Integer.toString(slices));
		prefs.put(FRAMES, Integer.toString(frames));
	}
