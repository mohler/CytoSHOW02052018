...
public class Projector implements PlugIn {
	...
	private boolean outlineObjects = false;
	...
	public void run(String arg) {
		...
		if (imp.getNFrames()>1 || imp.getNChannels()>1) {
			if (imp.getNSlices()>1)
		...
		if (isRGB) {
			ImagePlus imp2 = doRGBProjections(imp);
			if (imp2!=null)
				imp2.show();
		} else {
...
//			allTimePoints = false;
		...
		for (int c = 0; c < channels; c++) {
			for (int f = f1; f <=f2; f++) {
				...
				if (isRGB) {
					projImpD = doRGBProjections(impD);
//					projImpD.show();
				}
				else
...
//			finalSlices = 1;
//		}
		if (isRGB/*imp.getNChannels()>1*/)
			IJ.run( buildImp, 
				"Stack to Hyperstack...", "order=xyztc channels=" + finalChannels + " slices=" + finalSlices + " frames=" + finalFrames + " display=Color");
		else 
			IJ.run( buildImp, 
