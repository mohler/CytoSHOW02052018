...
public class QuickTime_TGrabWriter implements PlugIn, StdQTConstants, TextListener {
...
*/
	...
	public void writeMovie(ImagePlus imp, String path, int codecType, int codecQuality, int rate) throws QTException, IOException {
		...
		if (!includeTags) window.hide();
		
		
			for (int frame= firstT; frame<= lastT; frame++) {
				IJ.showProgress(frame-firstT, lastT-firstT+1);
				IJ.showStatus(frame -firstT+"/"+ (lastT-firstT+1) + " (" +IJ.d2s((frame-firstT)*100.0/(lastT-firstT+1),0)+"%)");
				...
				if (includeTags) {
					...
					imp.setTitle("***** " + title + " RECORDING!!  DO NOT TOUCH THIS WINDOW!!! *****");
					...
					try {
						...
						Image img1 = imp2.getImage();
						ColorProcessor ip = new ColorProcessor(img1);
	...
	String showHSDialog(ImagePlus imp) {
		...
		GenericDialog gd = new GenericDialog("Select Range");
