...
public class Slicer implements PlugIn, TextListener, ItemListener {
	...
	ImagePlus resliceHyperstack(ImagePlus imp) {
		...
		if (imp.getNChannels() == imp2.getNChannels()) {
			for (int c=1; c<=imp.getNChannels(); c++) {
				String sliceLabel = imp.getStack().getSliceLabel(c);
				for (int t=0; t<imp2.getNFrames(); t++) {
					imp2.getStack().setSliceLabel(sliceLabel, t*imp2.getNChannels()+c);
				}
			}
		}
	...
	ImageStack createOutputStack(ImagePlus imp, ImageProcessor ip) {
		 ...
		 ImagePlus imp2 = NewImage.createImage("temp", w2, h2, d2, bitDepth, flags, false);
