...
//		String strokeAlpha = Integer.toHexString(strokeColor.getAlpha());
		String linec = strokeColor!=null?"#"+Integer.toHexString(strokeColor.getRGB()):"none";
		if (linec.length()==7) {
			linec = "#"+"00"+linec.substring(1);
		}
...
//		}
		...
		if (overlayOptions) {
			...
			roi.setPosition(roi.getImage(), setPositions?1:0);
		}
		if (showListCoordinates)
		...
		if (isText) {
			TextRoi troi = (TextRoi)roi;
			Font font = troi.getCurrentFont();
			if ((int)strokeWidth!=0 && (int)strokeWidth!=font.getSize()) {
