...
public class Functions implements MacroConstants, Measurements {
	...
	Boolean checkIndex(int index, int lower, int upper) {
...
//			interp.error("Index ("+index+") is outside of the "+lower+"-"+upper+" range");
	...
	double roiManager() {
		...
		RoiManager rm = (RoiManager) (roiManager!=null?roiManager:WindowManager.getImage(impID).getRoiManager());
		if (rm==null)
			interp.error("Tag Manager not found");
		if (multiSelect)
		...
		if (twoArgCommand)
			...
			countOrIndex = rm.getListModel().getSize();
		else if (cmd.equals("index"))
			countOrIndex = rm.getList().getSelectedIndex();
		else {
			if (!rm.runCommand(cmd))
				interp.error("Invalid Tag Manager command");
		}
		return countOrIndex;			
	}
	...
	double setMultipleIndexes(RoiManager rm) {
		...
		int count = rm.getListModel().getSize();
	...
	private String ijCall() {
		...
		else if (name.equals("escapePressed"))
			{interp.getParens(); return ""+IJ.escapePressed();}
		else
	...
	double overlaySetPosition(Overlay overlay) {
		...
		if (size==0)
			return Double.NaN;
		if (nargs==1)
			overlay.get(size-1).setPosition(IJ.getImage(), n);
		else if (nargs==3)
