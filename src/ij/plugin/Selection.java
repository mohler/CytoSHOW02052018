...
public class Selection implements PlugIn, Measurements {
	...
	*/
	...
	void addToRoiManager(ImagePlus imp) {
		...
		Frame frame = imp.getRoiManager();
		if (frame==null)
			IJ.run("Tag Manager...");
		if (imp==null) return;
		Roi roi = imp.getRoi();
		if (roi==null) return;
		frame = imp.getRoiManager();
		if (frame==null || !(frame instanceof RoiManager))
			IJ.error("Tag Manager not found");
	...
	void updateInRoiManager(ImagePlus imp) {
		if (IJ.macroRunning() &&  Interpreter.isBatchModeRoiManager())
			IJ.error("run(\"Update in Manager\") may not work in batch mode macros");
		Frame frame = imp.getRoiManager();
		if (frame==null)
			return;
			//IJ.run("Tag Manager...");
