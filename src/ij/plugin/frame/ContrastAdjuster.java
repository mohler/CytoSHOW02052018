...
ActionListener, AdjustmentListener, ItemListener {
	...
	static final String[] channelLabels = {"1. Red", "2. Green", "3. Blue", "4. Cyan", "5. Magenta", "6. Yellow", "All Channels"};
	static final String[] altChannelLabels = {"Channel 1", "Channel 2", "Channel 3", "Channel 4", "Channel 5", "Channel 6", "All Channels"};
	...
	public static void setInstance(Frame instance) {
		ContrastAdjuster.instance = instance;
	}
	...
	public void run(String arg) {
		...
		balance = (balance || (IJ.getImage().isComposite() && IJ.getImage().getNChannels()>=1));
		if (windowLevel)
			setTitle("W&L");
		else if (balance) {
			setTitle("Display Channels");
			channels = 4;
		}
		...
		//setup choice early to get right plot width
		if (balance) {
			choice = new Choice();
			addBalanceChoices();
		}
		...
		// plot
		if (balance)
			plot = new ContrastPlot(choiceWidth);
		else 
...
//				addLabel("Contrast", null);
		...
		if (choice != null) {
			...
			channels = choice.getItemCount()-1;
		}
	}
			...
			if (imp.getWindow().createSubtitle().matches(".*\\[.*\\].*")) {
				...
				for (int i=0; i<imp.getNChannels(); i++) {
					...
					if (choice.getItem(choice.getItemCount()-1).length()*9 > choiceWidth) {
						...
						choice.add(shortenedItem);
					}
				}
				choice.addItem("All Channels");
				imp.setPositionWithoutUpdate(currentChannel, imp.getSlice(), imp.getFrame());
			} else {
				for (int i=0; i<imp.getNChannels(); i++) {
					choice.addItem("Channel "+ (i+1));
					if (choice.getItem(choice.getItemCount()-1).length()*9 > choiceWidth)
						choiceWidth = choice.getItem(choice.getItemCount()-1).length()*9;
				}
				choice.addItem("All Channels");
			}
		} else {
		...
		if (balance) {
			if (!imp.getWindow().isVisible() || imp == null || imp.getWindow() == null) {
				...
				if (choice.getItemCount()!= imp.getNChannels()+1 || ( !(imp.getWindow().createSubtitle().contains(choice.getItem(imp.getChannel()-1).substring(4)))
					...
					choice.removeAll();
					addBalanceChoices();
					choice.select(imp.getNChannels());
					this.setBackground(defaultBkgdColor);
					channels = imp.getNChannels();
				}
				if (choice.getSelectedIndex() <imp.getNChannels())
					channels = imp.getNChannels();
				else
					channels = imp.getNChannels();
	...
	void plotHistogram(ImagePlus imp) {
		...
		if (balance && (channels==4 || channels==2 || channels==1) && imp.getType()==ImagePlus.COLOR_RGB) {
			...
			if (range!=0 && imp.getProcessor().getMax()==Math.pow(2,range)-1 && !(imp.getCalibration().isSigned16Bit())) {
				...
				if (choice != null && choice.getSelectedIndex() < choice.getItemCount()-1)
					stats = imp.getChannelProcessor().getStatistics();
				else
					if (imp instanceof CompositeImage)
						stats = ((CompositeImage)imp).getProcessor().getStatistics();
					else
						stats = imp.getProcessor().getStatistics();
		}
		color = Color.gray;
		if (imp.isComposite() && !(balance && choice.getSelectedIndex()==imp.getNChannels()))
	...
	public synchronized  void itemStateChanged(ItemEvent e) {
		...
		if (imp!=null && imp.isComposite()) {
			if (index+1<=imp.getNChannels()) {
				...
				color = Color.gray;
				if (imp.isComposite() && balance && !(index==imp.getNChannels())) {
					color = ((CompositeImage)imp).getChannelColor();
					if (color != Color.gray)
...
class ContrastPlot extends Canvas implements MouseListener {
	...
	public ContrastPlot() {
		new ContrastPlot(128);
	}

	public ContrastPlot(int width) {
		this.width = width;
		addMouseListener(this);
		setSize(this.width+1, height+1);
	}
		...
		if (histogram!=null) {
			if (os==null && hmax!=0) {
				...
				for (int i = 0; i < width; i++)
					osg.drawLine(i, height, i, height - ((int)(height * histogram[i*256/width])/hmax));
				osg.dispose();
			}
