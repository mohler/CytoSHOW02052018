...
public class RemoteMTVSHandler {
	...
	public static void main(String args[]) {
		...
		new RemoteMTVSHandler(args);
	}

	public static RemoteMTVSHandler build(String url, String portOffset, String pathsThenSlices, 
		...
		return new RemoteMTVSHandler(argArrayList.toArray(new String[argArrayList.size()]));
	}

	public RemoteMTVSHandler(String args[]) {
				...
				if (jpegQuality < 100 ) { 
					...
					if (((RemoteMTVirtualStack)stack).selectedSlice == ((((StackWindow)imp2.getWindow()).zSelector.getValue()-1) * stkNSlices)
						...
						Thread thread = new Thread(new Runnable() {
							public void run() {
								RemoteMTVSHandler.this.jpegQuality=10*jpegQuality;
						...
						RemoteMTVSHandler.this.resolutionToGet = 1;
						RemoteMTVSHandler.this.jpegQuality=1;
						burnInComplete = false;
					...
					if ( (((RemoteMTVirtualStack)stack).selectedSlice == ((((StackWindow)imp2.getWindow()).zSelector.getValue()-1) * stkNSlices)
							...
							?((StackWindow)imp2.getWindow()).cSelector.getValue()
									:1)) 
									&& RemoteMTVSHandler.this.resolutionToGet < stkNChannels) {
						if (RemoteMTVSHandler.this.resolutionToGet == stkNChannels-1)
							burnInComplete = true;
						Thread thread = new Thread(new Runnable() {
							public void run() {
								RemoteMTVSHandler.this.resolutionToGet++;
	...
	public class RemoteMTVirtualStack extends VirtualStack {
		...
		public ImageProcessor getProcessor( int slice) {
			...
			if (moviePathNames[0].substring(moviePathNames[0].lastIndexOf("/")).startsWith("/SW_")
				...
				if (this.flipSingleMovieStackOrder[0])
					slice = RemoteMTVSHandler.this.getImagePlus().getNSlices()-(slice-1);
			} else {
				if (this.flipSingleMovieStackOrder[(slice-1)%stkNChannels]) {
					int base = slice-((slice-1)%(stkNChannels*stkNSlices));
					int nZ = RemoteMTVSHandler.this.getImagePlus().getNSlices();
			...
			if (adjustedSlice <0 ) {
				...
				if (dejaVuIPLinkedHashMap.get(adjustedSlice) != null) {
					...
					if (RemoteMTVSHandler.this.getImagePlus() != null)
						win = RemoteMTVSHandler.this.getImagePlus().getWindow();
		...
		public void setStretchToFitOverlay(boolean stretchToFitOverlay) {
			RemoteMTVSHandler.this.stretchToFitOverlay = stretchToFitOverlay;
		}
		...
		public void setEightBit(boolean eightBit) {
			RemoteMTVSHandler.this.eightBit = eightBit;
		}
		...
		public void setViewOverlay(boolean viewOverlay) {
			RemoteMTVSHandler.this.viewOverlay = viewOverlay;
		}
		...
		public void setSideSideStereo(boolean sideSideStereo) {
			RemoteMTVSHandler.this.sideSideStereo = sideSideStereo;
		}
		...
		public void setRedCyanStereo(boolean redCyanStereo) {
			RemoteMTVSHandler.this.redCyanStereo = redCyanStereo;
		}
		...
		public void setGrid(boolean grid) {
			RemoteMTVSHandler.this.grid = grid;
		}

		public void setHorizontal(boolean horizontal) {
			RemoteMTVSHandler.this.horizontal = horizontal;
		}
