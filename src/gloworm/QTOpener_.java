...
/**
	...
	This plugin is a version of Wayne Rasband's Movie_Opener
	plugin, modified to allow 2 modes of operation: interactive
	(for users) and noninteractive (for plugins that need to open
	QT Movies without user intervention).
*/
public class QTOpener_ implements PlugIn, QDConstants, StdQTConstants, MovieDrawingComplete {
	...
	/*Does about what the catch blocks in the original plugin did.*/
	...
	/*it can't be another kind of Throwable, otherwise it would
          have been checked (and been a compile-time error in
          orig. plugin)*/
    }
    public void run(String args) {
	/*The frontend for running this plugin interactively.*/
	try {
	    QTSession.open();
	...
	//The backend for the public run methods.
	try {
    ...
    public void runNoninteractively(String filename, String dir, boolean grayscale) {
	/*Opens file named filename in directory dir, in grayscale if grayscale argument is true.*/
	try {
	    
	    if (!QTSession.isInitialized())
		//run() didn't open the session already--do it now
		QTSession.open();
